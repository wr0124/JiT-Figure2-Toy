...
`toyD2_base/flow_D16_eps.gif` can be generated by running `toyD2_base/run_gif.sh` (`eps-pred`).
...
The scaling factor γ is introduced to control the strength of the orthogonal high-dimensional signal relative to the buried spiral signal, ensuring that the signal ratio is determined by the parameter ρ rather than by the ambient dimension D.
...
<a href="./toy_highdim/gamma_vs_rho.png"><img src="./toy_highdim/gamma_vs_rho.png" alt=""></a>

### Relation between ρ and γ

The strength of the orthogonal high-dimensional signal is controlled through the **energy ratio**
...
<p align="center">
<img src="https://latex.codecogs.com/svg.image?\rho=C\gamma^2,\qquad\gamma=\sqrt{\frac{\rho}{C}}" />
</p>

Therefore

<p align="center">
<img src="https://latex.codecogs.com/svg.image?\gamma\propto\sqrt{\rho}" />
</p>

Taking logarithms gives

<p align="center">
<img src="https://latex.codecogs.com/svg.image?\displaystyle\log\gamma=\frac{1}{2}\log\rho-\frac{1}{2}\log%20C" />
</p>

so on a **log–log plot**, the relationship between \( \log \gamma \) and \( \log \rho \) is **linear with slope \(1/2\)**.



<a href="./toy_highdim/outputs/flow_matching_pred_all_rho_ordered.png"><img src="./toy_highdim/outputs/flow_matching_pred_all_rho_ordered.png" alt="flow_matching_pred_all_rho_ordered.png"></a>

When **ρ is small**, most of the data energy lies in the 2D spiral subspace.  
As a result, the dataset remains **effectively low-dimensional**, even when the ambient dimension \(D\) is large.

As **ρ increases**, the orthogonal component \(u_\perp\) contributes more energy.  
This introduces signal across many directions in the \(D-2\) dimensional orthogonal space, increasing the **intrinsic dimension** of the data.

Consequently, the prediction task becomes significantly harder for an **under-complete model** (e.g., a 256-dimensional MLP operating in \(D=512\)), because the model must represent meaningful structure across many more directions of the ambient space.
